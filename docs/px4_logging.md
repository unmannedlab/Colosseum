...
Colosseum can capture mavlink log files if you add the following to the PX4 section of your `settings.json` file:
...
Colosseum will create a timestamped log file in this folder for each "armed/disarmed" flight session.
...
For example, Colosseum/PX4 flight may misbehave if you run it on an under powered computer.
...
- `hil_gps.alt` - the simulated altitude sent from Colosseum to PX4
- `telemetry.update_rate` - the rate Colosseum is performing the critical drone update loop in updates per second.
- `telemetry.update_time` - the average time taken inside Colosseum performing the critical drone update loop.
...
What we see then with these metrics is that things started off nicely, with nice flat altitude, high update rate in the 275 to 300 fps range, and a nice low update time inside Colosseum around 113 microseconds, and a nice low actuation delay in the round trip from PX4.  The actuator controls also stabilize quickly to a nice flat line.
...
But then we see actuator controls go crazy, a huge spike in actuation delay, and around this time we see a message from Colosseum saying `lockstep disabled`.  A delay over 100 millisecond triggers Colosseum into jumping out of lockstep mode and the PX4 goes nuts and the drone crashes.
...
then it means you are running Colosseum on a computer that does not have enough horsepower.
